...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if ('job_type' in body)   allowed.job_type   = String(body.job_type)
  if ('daily_rate' in body) allowed.daily_rate = Number(body.daily_rate) || 0
  if ('phone' in body)      allowed.phone      = body.phone || null
  if ('status' in body)     allowed.status     = String(body.status)
  if ('rating' in body)     allowed.rating     = Math.min(5, Math.max(0, Number(body.rating)))
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  const admin     = createAdminClient()
  const companyId = getCompanyId()
  const { error } = await admin.from('con_workers').delete().eq('id', params.id).eq('company_id', companyId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
