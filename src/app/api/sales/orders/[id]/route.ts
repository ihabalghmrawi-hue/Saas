...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error) return Errors.notFound('الطلب')
  return ok(data)
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { action, approved_by } = body as Record<string, unknown>

  if (action === 'approve') {
    const result = await domain.engines.order.approveOrder(id, approved_by as string)
    if (!result.ok) return Errors.badRequest(result.error)
    return ok(result.data)
  }
  return Errors.badRequest('إجراء غير صالح. استخدم approve')
}
