...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const company_id = _req.headers.get('x-tenant-id') || getCompanyId()
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    if (!action) {
      return NextResponse.json({ error: 'الإجراء مطلوب (post | void | reverse)' }, { status: 400 })
    }

    if (action === 'post') {
      const result = await postJournalEntry(supabase, params.id, approved_by)
      if (!result.ok) return NextResponse.json({ error: result.error }, { status: 422 })
      return NextResponse.json(result)
    }

    if (action === 'void') {
      const result = await voidJournalEntry(supabase, params.id)
      if (!result.ok) return NextResponse.json({ error: result.error }, { status: 422 })
      return NextResponse.json(result)
    }

    if (action === 'reverse') {
      const result = await reverseJournalEntry(supabase, params.id, company_id, reason)
      if (!result.ok) return NextResponse.json({ error: result.error }, { status: 422 })
      return NextResponse.json(result)
    }
