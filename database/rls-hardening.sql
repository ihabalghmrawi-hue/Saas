...
CREATE POLICY "companies: member can read own"
  ...
  USING (id::text = current_company_id());
...
CREATE POLICY "memberships: user sees own"
  ...
  USING (user_id::text = auth.uid()::text OR company_id::text = current_company_id());

CREATE POLICY "memberships: user can insert own"
  ON memberships FOR INSERT
  WITH CHECK (company_id::text = current_company_id());

-- ── COMPANY_SETTINGS ─────────────────────────────────────────────────────────

CREATE POLICY "company_settings: tenant isolation"
  ON company_settings FOR ALL
  USING (company_id::text = current_company_id())
  WITH CHECK (company_id::text = current_company_id());
...
CREATE POLICY "role_permissions: read via role"
  ...
  USING (
    role_id IN (
      SELECT id FROM staff_roles WHERE company_id::text = current_company_id()
    )
  );
...
CREATE POLICY "sale_items: tenant isolation"
  ...
  USING (
    sale_id IN (SELECT id FROM sales WHERE company_id::text = current_company_id())
  )
  WITH CHECK (
    sale_id IN (SELECT id FROM sales WHERE company_id::text = current_company_id())
  );
...
CREATE POLICY "purchase_items: tenant isolation"
  ...
  USING (
    purchase_id IN (SELECT id FROM purchases WHERE company_id::text = current_company_id())
  )
  WITH CHECK (
    purchase_id IN (SELECT id FROM purchases WHERE company_id::text = current_company_id())
  );
...
CREATE POLICY "journal_entry_lines: tenant isolation"
  ...
  USING (
    journal_entry_id IN (
      SELECT id FROM journal_entries WHERE company_id::text = current_company_id()
    )
  )
  WITH CHECK (
    journal_entry_id IN (
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='fiscal_years') THEN
    EXECUTE $pol$ CREATE POLICY "fiscal_years: tenant isolation" ON fiscal_years FOR ALL
      USING (company_id::text = current_company_id()) WITH CHECK (company_id::text = current_company_id()) $pol$;
  END IF;
